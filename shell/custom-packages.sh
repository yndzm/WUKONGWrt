...
# 进阶设置 by sirpdboy 
# 当luci-app-kucat-config插件开启时 需排除冲突项 luci-app-argon-config和luci-i18n-argon-config-zh-cn 减号代表排除
CUSTOM_PACKAGES="$CUSTOM_PACKAGES luci-app-kucat-config luci-kucat-config-zh-cn -luci-app-argon-config -luci-i18n-argon-config-zh-cn"
# 网络测速 by sirpdboy 
